<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      ...
      <right-toolbar :showSearch.sync="showSearch" @queryTable="fetchQCategory"></right-toolbar>
    </el-row>
    ...
    <div style="margin-top: 5px">
      ...
      >
        ...
        <el-table-column prop="questionCount" align="center" label="面试题数量"/>
    ...
    <el-dialog center :title="title" :visible.sync="dialogFormVisible">
      <el-form :rules="rules" ref="dataForm" :model="qCategory" >
        <el-form-item label="分类名" prop="name" :label-width="formLabelWidth">
          <el-input v-model="qCategory.name"></el-input>
        </el-form-item>
        <el-form-item label="排序" prop="sort" :label-width="formLabelWidth">
          <el-input v-model="qCategory.sort"></el-input>
        </el-form-item>
      </el-form>
...
<script>
import {fetchQCategory, add, info, update, deleteBatch, top,remove} from '@/api/qCategory'
...
export default {
  ...
  computed: {
    ...
    canAdd: function () {
      return hasAuth(this.pres, '/system/qCategory/add')
    },
    canDel: function () {
      return hasAuth(this.pres, '/system/qCategory/delete')
    },
    canDeleteBatch: function () {
      return hasAuth(this.pres, '/system/qCategory/deleteBatch')
    },
    canUpdate: function () {
      return hasAuth(this.pres, '/system/qCategory/update')
    },
    canTop: function () {
      return hasAuth(this.pres, '/system/qCategory/top')
    },
  },
  methods: {
    fetchQCategory: function () {
      fetchQCategory(this.params).then(res => {
    ...
    submit: function () {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          if (this.isEditForm) {
            update(this.qCategory).then(res => {
    ...
    handleCreate: function () {
      this.isEditForm = 0
      this.qCategory = this.getFormObject()
      this.title = "新增分类"
